...
#include "EtherMACFullDuplexGA3.h"
...
Define_Module(EtherMACFullDuplexGA3);

 EtherMACFullDuplexGA3:: EtherMACFullDuplexGA3()
{
}

void  EtherMACFullDuplexGA3::initialize(int stage)
{
    EtherMACBase::initialize(stage);

    if (stage == INITSTAGE_LOCAL) {
        if (!par("duplexMode").boolValue())
            throw cRuntimeError("Half duplex operation is not supported by  EtherMACFullDuplexGA3, use the EtherMAC module for that! (Please enable csmacdSupport on EthernetInterface)");
    }
...
void  EtherMACFullDuplexGA3::initializeStatistics()
{
...
void  EtherMACFullDuplexGA3::initializeFlags()
{
...
void  EtherMACFullDuplexGA3::handleMessage(cMessage *msg)
{
    if (!isOperational) {
...
void  EtherMACFullDuplexGA3::handleSelfMessage(cMessage *msg)
{
...
void  EtherMACFullDuplexGA3::startFrameTransmission()
{
    ASSERT(curTxFrame);
...
void  EtherMACFullDuplexGA3::processFrameFromUpperLayer(EtherFrame *frame)
{
...
void  EtherMACFullDuplexGA3::processMsgFromNetwork(cPacket *pk)
{
    EtherTraffic *msg = check_and_cast<EtherTraffic *>(pk);

    EV_INFO << msg << " received." << endl;
    if ((!connected || disabled)||((strcmp(this->getParentModule()->getParentModule()->getNedTypeName(),"inet.node.inet.StandardHost")==0)&&(strcmp(msg->getName(),"SetHLMAC")==0))){
    //if (!connected || disabled) {
...
void  EtherMACFullDuplexGA3::handleEndIFGPeriod()
{
...
void  EtherMACFullDuplexGA3::handleEndTxPeriod()
{
...
void  EtherMACFullDuplexGA3::finish()
{
    EtherMACBase::finish();
...
void  EtherMACFullDuplexGA3::handleEndPausePeriod()
{
    ASSERT(nullptr == curTxFrame);
...
void  EtherMACFullDuplexGA3::processReceivedDataFrame(EtherFrame *frame)
{
    // statistics
...
void  EtherMACFullDuplexGA3::processPauseCommand(int pauseUnits)
{
    if (transmitState == TX_IDLE_STATE) {
...
void  EtherMACFullDuplexGA3::scheduleEndIFGPeriod()
{
    ASSERT(nullptr == curTxFrame);
...
void  EtherMACFullDuplexGA3::scheduleEndPausePeriod(int pauseUnits)
{
    ASSERT(nullptr == curTxFrame);
...
void  EtherMACFullDuplexGA3::beginSendFrames()
{
    if (curTxFrame) {
